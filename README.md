...
- on Render free tier, the Blueprint runs `npm run db:push` during the build step because `preDeployCommand` is not supported and shell access may be unavailable
...
4. The build step runs `npm run db:push` automatically before `npm run build`.
5. Then copy the Render service URL, which will look like `https://dailyshopclose-web.onrender.com`.
